        ...
        translations:
          meaning.translations.length > 0
            ? meaning.translations.map(
                translationField => translationField.translation,
              )
            : [],
        synonyms:
          meaning.synonyms.length > 0
            ? meaning.synonyms.map(synonymField => synonymField.synonym)
            : [],
        antonyms:
          meaning.antonyms.length > 0
            ? meaning.antonyms.map(antonymField => antonymField.antonym)
            : [],
        usageExamples:
          meaning.usageExamples.length > 0
            ? meaning.usageExamples.map(
                usageExamplesField => usageExamplesField.usageExample,
              )
            : [],
      };
    }),
    ...
    meanings: data.meanings.map(meaning => {
      return {
        ...
        translations: meaning.translations
          ? meaning.translations.map(translation => ({ translation }))
          : [],
        ...
        usageExamples: meaning.usageExamples
          ? meaning.usageExamples.map(usageExample => ({ usageExample }))
          : [],
      };
    }),
  };
...
export const wordDataToWordDataPayload = (data: IWord): IUserWordPayload => {
  return {
    ...
    normalizedWord: data.word.toLowerCase(),
    transcription:
      data.transcription.uk || data.transcription.us
        ? {
