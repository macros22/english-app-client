...
export const WordForm = ({
  ...
  formValues,
  wordId,
}: WordFormProps): JSX.Element => {
  const { skip, wordsPerPageCount } = usePagination();
  ...
  return (
    <>
      <Form success error size="large">
          ...
          render={({ field }) => (
            <Form.Select
              ...
              onChange={(_, { value }) =>
                setValue(
                  'studyStatus',
                  value ? (value as WordStudyStatus) : WordStudyStatus.Learn,
                )
              }
        ...
        <Tab
          panes={meaningsFields.map((_, meaningIndex) => {
            return {
